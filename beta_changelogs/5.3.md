...
## Bug Fixes
- Fixed blue trimmed golden boots showing up as copper boots in 1.21-1.21.4
- Fixed blue trimmed golden boots showing as a blank tile in 1.21.5-1.21.8
  ([#312](<https://github.com/NeunEinser/bingo/issues/312>))

# 5.3-rc3

## Bug Fixes
- Fixed item pool generation not waiting until marker in lobby is loaded
  - This would cause card generation to be completely broken on servers after
    a restart.
- Fixed some remaining error logs caused by item frames on initial load
- Fixed item pool being generated twice on initial load
  - This didn't actually cause any visible bugs, but was just unnecessary.
- Fixed spawn preview generating too high when generating a card from seed
  and inputting the same seed that is currently loaded
...
- Updated pack and world icon to align with the new seed generation

## Bug Fixes
- Fixed a critical bug were the item pool was re-generated every tick, causing
  massive strain on TPS and other issues like item pool display chests not
  appearing automatically
- Actually fixed clock still spinning in game dimension
- Fixed setting signs potentially getting desynced after a lobby update
- Fixed pre-generation and target time signs not updating immediately after
  changing their value
- Fixed versions prior to 1.21.5 showing a non-existing translation string for
  the target time sign when it is set to a time.
