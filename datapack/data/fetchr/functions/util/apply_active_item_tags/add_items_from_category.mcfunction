...
execute if score $apply_tags.item_does_not_exists fetchr.tmp matches 1 run data modify storage tmp.fetchr:apply_active_item_tags activeItems[-1].weightDenom set from storage tmp.fetchr:apply_active_item_tags categories[-1].totalItemWeight
...
data modify storage tmp.fetchr:apply_active_item_tags itemCategories set from storage tmp.fetchr:apply_active_item_tags activeItems[-1].categories
