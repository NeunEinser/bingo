...
execute as @e[y=-64, dx=16, dz=16, dy=384] run function fetchr:game/start/pre_gen/entities/process_entity

tellraw NeunEinser ["ent", {"score": {"name": "$game_start/pre_gen/entities.i", "objective": "fetchr.tmp"}}, ": ", {"score": {"name": "$game_start/pre_gen/entities.x", "objective": "fetchr.tmp"}}, ", ", {"score": {"name": "$game_start/pre_gen/entities.z", "objective": "fetchr.tmp"}}]
execute if score $game_start/pre_gen/entities.i fetchr.tmp matches 9 positioned ~-16 ~ ~16 run function fetchr:game/start/create_skybox/exec
