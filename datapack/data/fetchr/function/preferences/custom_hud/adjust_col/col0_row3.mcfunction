...
execute \
	if score @s fetchr.pref matches 35 \
	run data \
...
execute \
	if score @s fetchr.pref matches 36 \
	run data \
...
execute \
	if score @s fetchr.pref matches 36..37 \
	run data \
		modify storage fetchr:custom_hud current_player.components[3] \
		set from storage fetchr:custom_hud current_player.components[4]
execute \
	if score @s fetchr.pref matches 36..37 \
	run data modify storage fetchr:custom_hud current_player.components[4] set value { id: "fetchr:empty" }