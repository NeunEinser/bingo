...
execute if score $blind_mode fetchr.state matches 1 run data modify storage fetchr:card slots[-1].display set value '{"text": "\\u0013", "font": "fetchr:space"}'
...
# set item to frame
execute unless score $blind_mode fetchr.state matches 1 as @e[type=minecraft:item_frame, tag=fetchr.card_frame] if score @s fetchr.lobby_card_frame_id = $card_gen.slot fetchr.tmp run data modify entity @s Item set from storage tmp.fetchr:card_generation items[-1].item
