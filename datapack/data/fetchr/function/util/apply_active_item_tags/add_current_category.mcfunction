...
execute store result score $apply_tags/add_item_cat.incorrect_cat fetchr.tmp run data modify storage tmp.fetchr:apply_active_item_tags category set from storage tmp.fetchr:apply_active_item_tags item_categories[-1].id

execute unless score $apply_tags/add_item_cat.incorrect_cat fetchr.tmp matches 1 run data modify storage tmp.fetchr:apply_active_item_tags activeItems[-1].activeCategories append from storage tmp.fetchr:apply_active_item_tags item_categories[-1]

data remove storage tmp.fetchr:apply_active_item_tags item_categories[-1]
