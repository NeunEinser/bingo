...
execute \
	if score $start_game.countdown fetchr.tmp matches 1.. \
	run tellraw @a[predicate=fetchr:is_in_game] {\
		"score": { "name": "$start_game.countdown", "objective": "fetchr.tmp" }\
	}
execute \
	if score $start_game.countdown fetchr.tmp matches 1.. \
	as @a[predicate=fetchr:is_in_game] \
	at @s \
	run playsound minecraft:block.note_block.pling voice @s ~ ~ ~ 1 2
