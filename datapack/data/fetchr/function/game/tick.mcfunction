...
execute \
	...
	at @s \
	run function #fetchr:game/player_tick
...
execute \
	store result score $game/tick.player_count fetchr.tmp \
	if entity @a[gamemode=survival, predicate=fetchr:is_in_game]
#>
...
execute \
	if score $game/tick.player_count fetchr.tmp matches 1.. \
...
execute \
	in fetchr:lobby \
	unless score $raw 91.timer.io = $last_tick_second fetchr.state \
