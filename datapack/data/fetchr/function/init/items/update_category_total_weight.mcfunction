...
scoreboard players operation $init/items/category.total_weight fetchr.tmp \
	+= $init/items/category.item_weight fetchr.tmp
...
execute \
	...
	run function fetchr:init/items/update_category_total_weight \
		with storage tmp.fetchr:init item_categories[-1]