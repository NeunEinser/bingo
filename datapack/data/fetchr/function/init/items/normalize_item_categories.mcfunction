#> fetchr:init/items/normalize_item_categories
#
...
execute \
	store result score $init/items/normalize.weight fetchr.tmp \
	run data get storage tmp.fetchr:init category.weight
execute \
	store result storage tmp.fetchr:init category.weight int 1 \
	run scoreboard players get $init/items/normalize.weight fetchr.tmp
...
execute \
	if data storage tmp.fetchr:init category.id \
	if score $init/items/normalize.weight fetchr.tmp matches 1.. \
	run data \
		modify storage tmp.fetchr:init items[-1].categories \
		append from storage tmp.fetchr:init category

# loop
data remove storage tmp.fetchr:init item_categories[-1]
execute \
	if data storage tmp.fetchr:init item_categories[0] \
	run function fetchr:init/items/normalize_item_categories