...
execute \
	if data storage tmp.fetchr:init/update_lobby structures[-1].override_id \
	unless data storage tmp.fetchr:init/update_lobby old_structures[-1].override_id \
	run scoreboard players set $init/lobby/update.should_update fetchr.tmp 1
...
execute \
	...
	run data \
		modify storage tmp.fetchr:init/update_lobby data_comparision \
		set from storage tmp.fetchr:init/update_lobby old_structures[-1].override_id
