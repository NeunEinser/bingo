...
#declare tag fetchr.pre_gen_cloud
...
summon minecraft:area_effect_cloud ~-320 ~ ~-320 {Duration: -1, Tags: ["fetchr.pre_gen_cloud"]}

# Make sure marker cloud is on chunk coords 0 0
#>
# @private
#declare score_holder $pre_gen.x
execute store result score $pre_gen.x fetchr.tmp run data get entity @e[type=minecraft:area_effect_cloud, tag=fetchr.pre_gen_cloud, limit=1] Pos[0]
#>
# @private
#declare score_holder $pre_gen.z
execute store result score $pre_gen.z fetchr.tmp run data get entity @e[type=minecraft:area_effect_cloud, tag=fetchr.pre_gen_cloud, limit=1] Pos[2]
#>
# @private
...
execute store result entity @e[type=minecraft:area_effect_cloud, tag=fetchr.pre_gen_cloud, limit=1] Pos[0] double 1 run scoreboard players get $pre_gen.x fetchr.tmp
execute store result entity @e[type=minecraft:area_effect_cloud, tag=fetchr.pre_gen_cloud, limit=1] Pos[2] double 1 run scoreboard players get $pre_gen.z fetchr.tmp
