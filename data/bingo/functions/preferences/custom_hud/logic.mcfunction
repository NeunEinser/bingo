...
data modify storage tmp.bingo:preferences/hud blankComponents append from storage bingo:custom_hud currentPlayer.components[{id: "bingo:blank"}]
...
execute if data storage tmp.bingo:preferences/hud hudComponent{id: "bingo:blank"} run data modify storage io.bingo:preferences menuOptions[{id: "bingo:custom_hud/adjust_col0"}].disabled set value true

# Work around for """WAI""" https://bugs.mojang.com/browse/MC-139625 :mad_neun:
data modify storage tmp.bingo:preferences/hud hudComponent set from storage bingo:custom_hud currentPlayer.components[6]
execute if data storage tmp.bingo:preferences/hud hudComponent{id: "bingo:blank"} run data modify storage io.bingo:preferences menuOptions[{id: "bingo:custom_hud/adjust_col1"}].disabled set value true