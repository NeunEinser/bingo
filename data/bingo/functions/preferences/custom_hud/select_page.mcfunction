...
#
# @input storage tmp.bingo:preferences/hud components
...
execute if score $preferences/hud.disabled bingo.tmp matches 1 if score $preferences/hud.page bingo.tmp = @s bingo.menu_page run data modify storage tmp.bingo:preferences/hud pageElements append from storage tmp.bingo:preferences/hud components[-1]
