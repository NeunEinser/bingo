...
data modify storage tmp.bingo:preferences/hud firstComponent set from storage bingo:custom_hud currentPlayer.components[6]
execute unless data storage tmp.bingo:preferences/hud firstComponent{id: "bingo:blank"} run scoreboard players set $preferences.next_page bingo.tmp 10
