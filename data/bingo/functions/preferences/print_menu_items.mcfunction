...
execute if data storage tmp.bingo:preferences menuOption{disabled: true} unless data storage tmp.bingo:preferences menuOption{hidden: true} run tellraw @s ["[", {"storage": "tmp.bingo:preferences", "nbt": "menuOption.textComponet", "interpret": true, "color": "gray", "hoverEvent": {"action": "show_text", "contents": {"storage": "tmp.bingo:preferences", "nbt": "menuOption.disabledHoverText", "interpret": true, "color": "yellow"}}}, "]"]
