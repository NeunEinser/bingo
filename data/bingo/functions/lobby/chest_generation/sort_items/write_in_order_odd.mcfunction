...
execute if score $chest_generation/sort.a bingo.tmp < $chest_generation/sort.b bingo.tmp run data modify storage temp:bingo.chest_generation mergeSort.odd[-1] append from storage temp:bingo.chest_generation category.items[-1]
data modify storage temp:bingo.chest_generation mergeSort.odd[-1] append from storage temp:bingo.chest_generation category.items[-2]
execute if score $chest_generation/sort.a bingo.tmp >= $chest_generation/sort.b bingo.tmp run data modify storage temp:bingo.chest_generation mergeSort.odd[-1] append from storage temp:bingo.chest_generation category.items[-1]

data remove storage temp:bingo.chest_generation category.items[-1]
data remove storage temp:bingo.chest_generation category.items[-1]

execute if data storage temp:bingo.chest_generation category.items[1] run function bingo:lobby/chest_generation/sort_items/write_in_order_odd