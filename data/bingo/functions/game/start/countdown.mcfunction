...
execute if score $start_game.countdown bingo.schedule matches 1.. run tellraw @a {"score":{"name": "$start_game.countdown", "objective": "bingo.tmp"}}
...
execute if score $start_game.countdown bingo.schedule matches 0 as @a[predicate=!bingo:is_in_lobby] run function bingo:game/start/start_game
