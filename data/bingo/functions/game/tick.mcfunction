...
execute store result score $game/tick.player_count bingo.tmp if entity @a[predicate=bingo:is_in_game]
...
execute store result score $game/tick.sleeping_players bingo.tmp if entity @a[predicate=bingo:is_in_game, nbt={SleepTimer: 100s}]
