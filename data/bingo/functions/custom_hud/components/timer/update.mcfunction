...
data modify storage temp:bingo.custom_hud component set value {textComponent:'{"storage":"neun_einser.timer:display","nbt":"hh:mm:ss","interpret":true}', changed: true}
...
execute if score $game_state bingo.state matches 1 run data modify storage temp:bingo.custom_hud component.textComponent set value '"0"'
...
data modify storage bingo:custom_hud components[{id: "bingo:timer"}] merge from storage temp:bingo.custom_hud component