...
execute unless score $custom_hud/timer.last bingo.tmp = $custom_hud/timer.cur bingo.tmp unless score $game_state bingo.state matches 0..2 if data storage bingo:custom_hud currentPlayer.components[{id: "bingo:timer"}] run scoreboard players set $custom_hud/timer.update_timer bingo.tmp 1
execute if score $game_state bingo.state matches 0..2 unless score $last_tick bingo.state matches 0 if data storage bingo:custom_hud currentPlayer.components[{id: "bingo:timer"}] run scoreboard players set $custom_hud/timer.update_timer bingo.tmp 1
