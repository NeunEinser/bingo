...
execute as @e[type=minecraft:area_effect_cloud, tag=bingo_egg, scores={bingo_slot_id=0..}, limit=1] run function bingo:item_detection/find_slot/main

execute if entity @s[tag=bingo_successful_got_item] run tellraw @a {"translate": "bingo.got_item", "with": [{"selector": "@s"}, {"translate": "item.minecraft.egg", "hoverEvent": {"action": "show_item", "contents": {"id": "minecraft:egg"}}}]}
