...
execute store success score $item_detect/flint.success bingo.tmp if data storage bingo:card slots[{item:{id:"bingo:flint"}}] run data modify storage bingo:card slots[{item:{id:"bingo:flint"}}].selected set value true
