...
execute if score $row bingo.tmp matches 0 if score $col bingo.tmp matches 0 if score $diag bingo.tmp matches 0 if score $items bingo.tmp matches 20 run tellraw @a {"translate": "bingo.got_20_no_bingo", "with": [{"storage": "neun_einser.timer:display", "nbt": "\"hh:mm:ss.s\"", "interpret": true}, {"selector": "@a[tag=bingo.in_current_team]"}]}