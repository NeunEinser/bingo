...
execute store result score $item_detect/tag_players.player bingo.tmp run data get storage bingo:card slots[{selected: true}].players[-1]
execute as @a if score @s bingo.id = $item_detect/tag_players.player bingo.tmp run tag @s add bingo.clear
data remove storage bingo:card slots[{selected: true}].players[-1]
