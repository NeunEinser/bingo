...
execute as @e[type=minecraft:area_effect_cloud, tag=bingo_diamond, scores={bingo_slot_id=0..}, limit=1] run function bingo:award_item/award_item

execute if entity @s[tag=bingo_successful_got_item] run tellraw @a {"translate": "bingo.got_item", "with": [{"selector": "@s"}, {"translate": "item.minecraft.diamond", "hoverEvent": {"action": "show_item", "contents": {"id": "minecraft:diamond"}}}]}
