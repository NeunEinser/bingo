...
execute store success score $apply_tags.category_has_not_tag bingo.tmp run data modify storage tmp.bingo:apply_active_item_tags currentTag set from storage tmp.bingo:apply_active_item_tags tags[-1]

data remove storage tmp.bingo:apply_active_item_tags tags[-1]
execute if score $apply_tags.category_has_not_tag bingo.tmp matches 1 if data storage tmp.bingo:apply_active_item_tags tags[0] run function bingo:util/apply_active_item_tags/check_current_tag